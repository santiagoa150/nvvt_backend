...
class NovaVentaOrderClient(OrderClient):
    ...
    async def build_product(self, provider: ProductProvider, product_url: StringValueObject) -> Product:
        ...
        try:
            ...
            catalog_price = raw_catalog_price.text.replace('$', '').replace('.', '').replace(',', '.').strip()

            raw_list_price = soup.find('span', class_='regular__pdp--listprice--price')
            list_price = raw_list_price.text.replace('$', '').replace('.', '').replace(',', '.').strip()

            product = Product.from_dict(ProductDict(
                image_url=image_url,
